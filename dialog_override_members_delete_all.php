<?php
/**
 * dialog_override_members_delete_all.php
 *
 * @package   PhpStorm
 * @file      dialog_override_members_delete_all.php
 * @author    gparkin
 * @date      07/24/2017
 * @version   7.0
 *
 * @brief     Called by toolbar_override_netpins.php to delete all members in override contact list.
 *
 */
...
$lib->debug_start('dialog_override_members_delete_all.html');
...
$query = "select * from cct7_override_netpins where netpin_id = " . $netpin_id;

if ($ora->sql2($query) == false)
{
    $lib->debug1(__FILE__, __FUNCTION__, __LINE__, "%s - %s", $ora->sql_statement, $ora->dbErrMsg);
    printf("List no longer exists.  netpin_id = %d", $netpin_id);
    exit();
}
...
if ($ora->fetch() == false)
{
    $lib->debug1(__FILE__, __FUNCTION__, __LINE__,
             "cct7_override_netpins record where netpin_id = %d no longer exists", $netpin_id);
    printf("List no longer exists.  netpin_id = %d", $netpin_id);
    exit();
}
...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteAll()
        {
            ...
            var url = 'ajax_dialog_override_members_delete_all.php';
...
<form name="f1">
    <table bgcolor="#ECE9D8" width="100%" cellspacing="2" cellpadding="2" style="color: black">
        <tr>
            <td align="center">
                <p>Confirm that you want to remove all members for NET group pin: <?php echo $netpin_no; ?></p>
            </td>
        </tr>
        <tr>
            <td align="center">
                <a class="green" title="Delete all members from this list?"
                   href="#" onclick="deleteAll()">Yes</a>
                <a class="red" title="No I do not want to remove any members. Close this dialog."
