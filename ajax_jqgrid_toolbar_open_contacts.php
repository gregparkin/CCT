<?php
/**
 * ajax_jqgrid_toolbar_open_contacts.php
 *
 * @package   PhpStorm
 * @file      ajax_jqgrid_toolbar_open_contacts.php
 * @author    gparkin
 * @date      03/24/2020
 * @version   1.0
 *
...
$lib->debug1(  __FILE__, __FUNCTION__, __LINE__, "HELLO WORLD!");
...
{
	$lib->debug1(__FILE__, __FUNCTION__, __LINE__, "strlen(what_netpin_no) > 0");
	...
	$query .= "     t.system_id, ";
	$query .= "     t.contact_netpin_no, ";
	$query .= "     t.contact_respond_by_date, ";
	$query .= "     t.contact_response_status, ";
	$query .= "     t.contact_response_date, ";
	$query .= "     t.contact_response_cuid, ";
	$query .= "     t.contact_response_name,  ";
	$query .= "     t.contact_send_page,  ";
	$query .= "     t.contact_send_email  ";
	$query .= "   from  ";
	$query .= "     cct7_contacts t ";
	$query .= "   where  ";
	$query .= "     t.system_id = " . $system_id . " and t.contact_netpin_no = '" . $what_netpin_no . "' ";
	$query .= "  ) tbl ";
}
else if ($what_tickets == 'approve')
{
	...
	$query .= "     cct7_contacts t ";
	$query .= "   where  ";
	$query .= "     t.system_id = " . $system_id . " and t.contact_netpin_no " . $user_groups . " and ";
	$query .= "     t.contact_response_status = 'WAITING' ";
	$query .= "  ) tbl ";
}
else
{
	...
	$query .= '  t.contact_send_page, ';
	$query .= '  t.contact_send_email ';
	$query .= 'from cct7_contacts t ';
	$query .= sprintf(" where t.system_id = %d) tbl ", $system_id);
}
...
else if ($what_tickets == 'approve')
{
	...
	$query .= "  cct7_contacts t ";
	$query .= "where  ";
	$query .= "  t.system_id = " . $system_id . " and t.contact_netpin_no " . $user_groups . " ";
	$query .= "order by " . $order_by . " " . $direction;
}
else
{
...
while ($ora->fetch())
{
	...
	$contact_respond_by_date =
		$lib->gmt_to_format(
			$ora->contact_respond_by_date,
			'm/d/Y T',
			$tz);
	...
	$row['id']                       = $ora->system_id . $ora->contact_netpin_no;  // (key)
	$row['system_id']                = $ora->system_id;                // hidden column (system_id)
	$row['contact_netpin_no']        = $ora->contact_netpin_no;        // Netpin
	$row['contact_approver_fyi']     = $ora->contact_approver_fyi;     // Notify Type
	$row['contact_respond_by_date']  = $contact_respond_by_date;       // Respond By

	if ($ora->contact_approver_fyi == "FYI")
	{
		$row['contact_response_status']  = "FYI-ONLY";
	}
	else
	{
	...
	$row['contact_response_date']    = $contact_response_date;         // Response Date
	$row['contact_response_name']    = $ora->contact_response_name;    // Responded By
	$row['contact_send_page']        = $ora->contact_send_page;        // Page On-Call
	$row['contact_send_email']       = $ora->contact_send_email;       // Send Email
