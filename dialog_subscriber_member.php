<?php
/**
 * dialog_subscriber_member.php
 *
 * @package   PhpStorm
 * @file      dialog_subscriber_member.php
 * @author    gparkin
 * @date      11/22/16
 * @version   7.0
 *
...
$lib->debug_start('dialog_subscriber_member.html');
...
$action       = $parm['action'];          // add, delete, delete_all
...
if ($group_id > 0)
{
    $query = sprintf("select * from cct7_subscriber_members where group_id = %d", $group_id);
    ...
    if ($ora->fetch() == false)
    {
        $lib->debug1(__FILE__, __FUNCTION__, __LINE__, "Record no longer available for group_id = %d", $group_id);
        exit();
    }

    $member_name = $ora->member_name;
}
...
<head>
    ...
    <script type="text/javascript">
        ...
        function callAjax(action, group_id)
        {
            ...
            //alert('dialog_subscriber_member, action = ' + action + ', group_id = ' + group_id);
            ...
            //
            // Prepare the data that will be sent to ajax_dialog_subscriber_member.php
            ...
            var url = 'ajax_dialog_subscriber_member.php';
