<?php
/**
 * ajax_jqgrid_override_netpins_master.php
 *
 * @package   PhpStorm
 * @file      ajax_jqgrid_override_netpins_master.php
 * @author    gparkin
 * @date      7/22/2017
 * @version   7.0
 *
 * @brief     Called by toolbar_override_netpins.php
 *
 */
...
$lib->debug_start('ajax_jqgrid_override_netpins_master.html');
...
function get()
{
	...
	$query = "select count(*) as total_records from cct7_override_netpins";
	...
	$query = "select * from cct7_override_netpins order by netpin_no ";
	...
	while ($ora->fetch())
	{
		...
		$row['create_date'] = $lib->gmt_to_format($ora->create_date, 'm/d/Y H:i T', $tz);
		$row['create_cuid'] = $ora->create_cuid;
		$row['create_name'] = $ora->create_name;
		$row['netpin_no']   = $ora->netpin_no;
