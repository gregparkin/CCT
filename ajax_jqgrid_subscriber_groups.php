 ...
 *
 * @package   PhpStorm
 * @file      ajax_jqgrid_subscriber_groups.php
 * @author    gparkin
 * @date      8/26/16
 * @version   7.0
 *
 * @brief     Retrieve all subscriber lists.
 *
 */
...
$lib->debug_start('ajax_jqgrid_subscriber_groups.html');
...
function get()
{
	...
	$query_part2 = "from ";
	$query_part2 .= "  cct7_subscriber_groups t ";
	$query_part2 .= "order by ";
	$query_part2 .= "  t.group_id";
	...
	{
		//
		// Get the number of members in the member list for this group.
		//
		$member_count = 0;
		...
		$query .= "from ";
		$query .= "  cct7_subscriber_members ";
		$query .= "where ";
		$query .= "  group_id = '" . $ora->group_id . "'";
		...
		if ($ora2->fetch())
		{
			$member_count = $ora2->total_records;
		}

		//
		// Get the number of servers in the server list for this group.
		//
		$server_count = 0;
		...
		$row['group_name']        = $ora->group_name;
		$row['create_date']       = $lib->gmt_to_format($ora->create_date, 'm/d/Y T', $tz);
