<?php
/**
 * ajax_dialog_override_members_remove.php
 *
 * @package   PhpStorm
 * @file      ajax_dialog_override_members_remove.php
 * @author    gparkin
 * @date      07/24/17
 * @version   7.0
 *
 * @brief     Called by dialog_override_netpins_remove to remove a selected override netpin group.
 *
 */
...
$lib->debug_start('ajax_system_remove.html');
...
foreach ($list as $id)
{
	$lib->debug1(__FILE__, __FUNCTION__, __LINE__, "foreach: list_system_id = %d", $id);

	//
	// Put together a list of servers we are removing.
	//
	if ($ora->sql("select * from cct7_list_systems where list_system_id = %d", $id) == false)
	{
		$lib->debug1(  __FILE__, __FUNCTION__, __LINE__, "%s - %s", $ora->sql_statement, $ora->dbErrMsg);
		$json['ajax_status']  = 'FAILED';
		$json['ajax_message'] = $ora->error;
	...
	$query = "delete from cct7_list_systems where list_system_id = " . $id;
	...
	$lib->debug1(__FILE__, __FUNCTION__, __LINE__,   "servers: %s", $servers);
