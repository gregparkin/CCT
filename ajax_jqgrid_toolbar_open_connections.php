<?php
/**
 * ajax_jqgrid_toolbar_open_connections.php
 *
 * @package   PhpStorm
 * @file      ajax_jqgrid_toolbar_open_connections.php
 * @author    gparkin
 * @date      6/29/16
 * @version   7.0
 *
...
$lib->debug_start('ajax_jqgrid_toolbar_open_connections.html');
...
$rowid        = isset($input->rowid)        ? $input->rowid        : '';

$system_id = 0;
$netpin_no = '0';

//
// Parse the rowid from substrid3. (i.e. mainGrid_CCT700000010_subgrid1_657_subgrid2_65751190)
//                                rowID: mainGrid_CCT700000002_subgrid1_325_subgrid2_17340  RowKey: 17340
//
$lib->debug1(__FILE__, __FUNCTION__, __LINE__,   "Parsing rowid: %s", $rowid);
...
if (strlen($rowid) > 0)
{
	...
	$lib->debug1(__FILE__, __FUNCTION__, __LINE__,   "cnt = %d", $cnt);

	if ($cnt == 6)
	{
		foreach ($arr as $item)
		{
			$lib->debug1(__FILE__, __FUNCTION__, __LINE__,   "parse item: %s", $item);
		}

		$system_id = trim($arr[3]);
		$netpin_no = trim($arr[5]);
	}
}
...
if (strlen($system_id) > 0)
	$where_clause = sprintf(" where system_id = %d and contact_netpin_no = '%s' ", $system_id, $netpin_no);
...
while ($ora->fetch())
{
	...
	$contact_respond_by_date = $lib->gmt_to_format($ora->contact_respond_by_date, 'm/d/Y',     $tz);

	if ($ora->contact_response_date == 0)
		$contact_response_date = '';
	else
		$contact_response_date = $lib->gmt_to_format($ora->contact_response_date,   'm/d/Y H:i T', $tz);

	// CONTACTS
	// ===========================================
	// contact_id              = (hidden)
	// system_id               = (hidden)
	// contact_netpin_no       = Netpin No
	// contact_connection      = Connection
	...
	// contact_apps_databases  = App or DB Name

	// | Netpin | Connections                | OS   | Usage      | Group | Type     | CSC Banner           | App or DB Name |
	// | 17340  | hhdnp29a-hcdnx11a-hvdnp63a | HPUX | PRODUCTION | PASE  |APPROVER  | Application Support  | APRIL

	$row['contact_id2']             = $ora->contact_id;             // hidden column (system_id) - key
	$row['system_id2']              = $ora->system_id;              // hidden column (system_id)
	$row['contact_netpin_no2']      = $ora->contact_netpin_no;      // Netpin No
	$row['contact_connection2']     = $ora->contact_connection;     // Connection
	$row['contact_server_os2']      = $ora->contact_server_os;      // OS
	$row['contact_server_usage2']   = $ora->contact_server_usage;   // Usage
	$row['contact_work_group2']     = $ora->contact_work_group;     // Group
	$row['contact_approver_fyi2']   = $ora->contact_approver_fyi;   // Type
	$row['contact_csc_banner2']     = $ora->contact_csc_banner;     // CSC Banner
	$row['contact_apps_databases2'] = $ora->contact_apps_databases; // App or DB Name
